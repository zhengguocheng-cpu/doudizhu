...
### 21:49 - Bug修复：新一轮开始后仍提示牌型不匹配

#### 🐛 问题描述
用户测试时发现：
- 玩家1出牌 → 玩家2不出 → 玩家3出牌压过
- 玩家1不出 → 新一轮应该开始
- 玩家3再次出牌时，提示"牌型不匹配"或"无效的牌型"
- 应该可以出任意牌型

#### 🔍 根本原因
前端没有监听`new_round_started`事件：
- 后端在2人pass后发送`new_round_started`事件
- 前端没有监听，导致`lastPlayedCards`没有被清空
- 验证时还在和上一轮的牌比较
...
#### 📝 修改内容
- `room-simple.js`：
  * 添加`new_round_started`事件监听
  * 实现`onNewRoundStarted()`方法
  * 清空上家出牌信息
  * 隐藏桌面上的牌
