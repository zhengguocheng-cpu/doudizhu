...
const constructorContainer = {
  ...
  resolve(TokenClass) {
    if (this.services.has(TokenClass)) {
      return this.services.get(TokenClass)();
    }
    throw new Error(`Service not found: ${TokenClass.name}`);
  }
};
...
symbolContainer.register(loggerSymbol, () => ({ log: (msg) => console.log(`[SYMBOL-LOG] ${msg}`) }));
const userServiceSymbol = Symbol('UserService');
symbolContainer.register(userServiceSymbol, () => ({ getUser: (id) => ({ id, name: 'Bob' }) }));
...
// Symbol解析
const logger2 = symbolContainer.resolve(loggerSymbol);
