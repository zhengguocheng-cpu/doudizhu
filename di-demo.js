...
// 结构化日志接口
class LogEntry {
  constructor(level, message, context, timestamp) {
    this.level = level;
...
class SimpleDependencyContainer {
  constructor() {
    ...
    this.singletonFactories = new Map();
    this.transientFactories = new Map();

    SimpleDependencyContainer.instance = this;
  }

  static getInstance() {
    if (!SimpleDependencyContainer.instance) {
      new SimpleDependencyContainer();
    }
    return SimpleDependencyContainer.instance;
  }
