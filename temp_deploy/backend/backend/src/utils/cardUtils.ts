...
export class CardUtils {
  ...
  // 获取牌型
  static getCardType(cards: Card[]): { type: string; level: number; value: number } {
    if (cards.length === 0) return { type: 'empty', level: 0, value: 0 };
  ...
  // 判断是否为三带一
  private static isThreeWithOne(cards: Card[]): boolean {
    if (cards.length !== 4) return false;
  ...
  // 判断是否为三带一对
  private static isThreeWithPair(cards: Card[]): boolean {
  ...
  // 判断是否为连对
  private static isStraightPair(cards: Card[]): boolean {
  ...
  // 判断是否为飞机
  private static isThreeOfAKindStraight(cards: Card[]): boolean {
  ...
  // 判断是否为飞机带翅膀
  private static isPlaneWithWings(cards: Card[]): boolean {
  ...
  // 判断是否为四带二
  private static isFourWithTwo(cards: Card[]): boolean {
  ...
  // 比较相同类型的牌
  private static compareSameTypeCards(cards1: Card[], cards2: Card[], type1: any): number {
